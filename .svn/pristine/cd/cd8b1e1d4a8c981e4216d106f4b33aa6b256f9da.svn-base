...
public class ResettableInputStream extends ReleasableInputStream {
	 ...
	 * position so that subsequent reads re-read the same bytes. This method
	 * works as long as the underlying file has not been closed.
	 * <p>
	...
	public void mark(int _) {
		try {
			...
			throw new EvsClientException("Failed to mark the file position", e);
		}
	}

	/**
	 * Repositions this stream to the position at the time the <code>mark</code>
	 * method was last called on this input stream. This method works as long as
	 * the underlying file has not been closed.
	 * <p>
	 * Note the creation of a {@link ResettableInputStream} would entail
	 * physically opening a file. If the opened file is meant to be closed only
	 * (in a finally block) by the very same code block that created it, then it
	...
	public static ResettableInputStream newResettableInputStream(File file, String errmsg) {
		try {
			...
			throw new EvsClientException(errmsg, e);
		}
	}
